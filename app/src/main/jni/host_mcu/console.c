...
#include "../pty/pty.h" // openpty on Android
...
{
    int e = errno;
    __android_log_print(ANDROID_LOG_ERROR, "beam_host_mcu", "Got error %d in %s: (%d)%s\n", rc, where, e, strerror(e));
}
...
console_setup(char *name)
{
    ...
    // Beam changed: force set flags (Some devices configured VERY wrong by default)
    set_flags(mfd);
    set_flags(sfd);
